...
// Import functions
import {
  isDataURL, isImage, isEmpty, head,
...
class TableCell extends React.Component {
  ...
  renderImage(value, parseImg = {}) {
    ...
    const shouldBeAnImg = isImg || isImage(value)
    if (shouldBeAnImg) {
  ...
  parseURLs(value, filterValue, parseImg) {
    ...
    if (parseImg && firstLink.type === 'url') {
      const imageValue = isDataURL(value) ? value : firstLink.value
      image = this.renderImage(imageValue, parseImg)
    }
    return (
