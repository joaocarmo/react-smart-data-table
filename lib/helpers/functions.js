...
const isObject = (obj) => obj && typeof obj === 'object' && obj.constructor === Object || false
...
export function generatePagination(activePage = 1, totalPages = 1, margin = 1) {
  ...
  if (totalPages > numPagesShow) {
    if (activePage >= gap) {
      ...
      if (activePage > gap) {
        pagination.push({ active: false, value: undefined, text: '...' })
      }
    }
...
export function parseDataForColumns(
  data = [], headers = {}, orderedHeaders = [], hideUnordered = false,
) {
  const columnsAdded = []
  const columns = []
...
export function isDataURL(url) {
  if (typeof url === 'string') {
    ...
    if (initData && initData === 'data') {
      if (restData) {
        const [mediaType] = restData.split(';')
        if (mediaType) {
