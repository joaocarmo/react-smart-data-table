...
// Import functions
import {
  debugPrint, fetchData, parseDataForColumns, parseDataForRows,
...
`
  ...
  getColumns(force = false) {
    ...
    const {
      data, headers, orderedHeaders, hideUnordered,
    } = this.props
    if (!force && !isEmpty(columns)) return columns
    if (isString(data)) {
      return parseDataForColumns(asyncData, headers, orderedHeaders, hideUnordered)
    }
  ...
  getRows() {
    ...
    if (isString(data)) {
      return sortData(filterValue, colProperties, sorting, parseDataForRows(asyncData))
    }
